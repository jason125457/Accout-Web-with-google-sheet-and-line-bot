 ...
 * 4. 【日期修復】以原生 Date 物件寫入試算表，確保呈現「yyyy/M/d 上午/下午 hh:mm:ss」真日期格式（靠右對齊）
 * 5. 【月份靠右】最右側「月份」欄位強制設定為靠右對齊
 * 6. 【乾淨回覆】LINE 回覆格式恢復為最習慣的簡潔四行版本
 * 7. 【Gemini 備援鏈】依序嘗試 gemini-3.1-flash-lite -> 3.5-flash-lite -> 2.5-flash-lite，thinkingBudget: 0 秒回
 * ==============================================================================
 */
...
// 極速模型備援鏈（官方 API 端點嚴格要求「全小寫」且以「-」連字號連接）
const GEMINI_MODELS = [
...
function callGemini(userText) {
  ...
  if (!apiKey || apiKey.startsWith('YOUR_') || apiKey.startsWith('請填入')) {
    throw new Error('未設定 GEMINI_API_KEY，請在指令碼屬性或程式碼中填入金鑰');
  }
...
function writeToSheet(item, category, amount) {
  ...
  /**
   * 寫入原生 Date 物件 now，Google 試算表視為真日期時間
   */
...
function testWriteToSheet() {
  ...
  try {
    const res = writeToSheet('便當', '生活', 120);
    Logger.log(`✅ 成功寫入！月份: ${res.month}`);
  } catch (err) {
